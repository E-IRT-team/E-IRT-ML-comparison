...
# p_grid for kNN
# empirical setting
p_grid = {'max_depth': [3, 6], 'learning_rate': [0.001, 0.01, 0.1], 'n_estimators': [100,200]}
##-----------------------------------------------------------------------------
outer_folds = 10
inner_folds = 3
##-----------------------------------------------------------------------------
...
#        inner_cv_scores = np.zeros([outer_folds])
        ...
        GBClassifier,clname = GradientBoostingClassifier(),"GB"
        ...
        for train, test in scenarios[group]:  # KFold.split is a generator object, outputs 2 ndarrays
            ...
            clf = GridSearchCV(estimator=GBClassifier, param_grid=p_grid, 
