...
# p_grid for kNN
# empirical setting
#p_grid = {'hidden_layer_sizes': [(25,25,25), (50,50,50), (100,100,100)], 'learning_rate_init': [0.0001, 0.001, 0.01, 0.1], 'alpha': [0.0001, 0.001, 0.01, 0.1]}
#p_grid = {'hidden_layer_sizes': [(10,10), (10,10,10), (20,20), (20,20,20)], 'solver': ['adam'], 'alpha': [0.00001, 0.0001, 0.001, 0.01, 0.1]}
#p_grid = {'hidden_layer_sizes': [(10,10,10), (20,20,20)], 'solver': ['adam'], 'alpha': [0.0001, 0.001, 0.01, 0.1]}
p_grid = {'hidden_layer_sizes': [(40,40,40), (20,20,20)], 'solver': ['adam'], 'alpha': [0.0001, 0.001, 0.01, 0.1]}
##-----------------------------------------------------------------------------
outer_folds = 10
inner_folds = 3
##-----------------------------------------------------------------------------
...
#        inner_cv_scores = np.zeros([outer_folds])
        ...
        for train, test in scenarios[group]:  # KFold.split is a generator object, outputs 2 ndarrays
            ...
            clf = GridSearchCV(estimator=MLP, param_grid=p_grid, 
                               cv=inner_cv, scoring='roc_auc',
                               n_jobs=-1, refit=True)  # set iid=False to avoid firing warnings, if it happens
